...
(function (window, $, undef) {
    ...
    $.createControlKnob = function (options) {
        ...
        for (t = 0, tlen = 28; t < tlen; t += 1) {
            band_list.push(document.createElement('span'));
        }
        $ck_band = $(band_list);
        ...
        // Create the jQueryfied control knob object (http://api.jquery.com/jQuery/#working-with-plain-objects)
        $control_knob_object = $({
            ...
            displayLabels: function (show) {
                show = !!show;
                $ck_min_label.toggle(show);
                $ck_max_label.toggle(show);
            },
            displayBands: function (show) {
                $ck_bands_wrap.toggle(!!show);
            },
            displayValue: function (show) {
        ...
        (function () {
            ...
            refreshControls = function refreshControls(calculate_angle) {
                ...
                active_bands = (Math.floor(angle / 10) + 1);
            ...
            */
            ...
            control_knob_object.enable = function () {
                if (disabled === true) {
                    disabled = false;
                    $ck_wrap
                        ...
                        .on('mousedown touchstart mousewheel mousewheel', ckWrapEventHandler);
                }
                return control_knob_object;
            };
            control_knob_object.disable = function () {
                if (disabled === false) {
                    disabled = true;
                    if (active) {
                    ...
                    $ck_wrap
                        .addClass('disabled')
                        .removeAttr('tabindex')
                        .off('mousedown touchstart mousewheel mousewheel', ckWrapEventHandler)
            ...
            control_knob_object.on = function () {
                applier($_proto.on, $control_knob_object, arguments);
                return control_knob_object;
            };
            control_knob_object.one = function () {
                applier($_proto.one, $control_knob_object, arguments);
                return control_knob_object;
            };
            control_knob_object.off = function () {
            ...
            control_knob_object.reset = function (hard) {
                var i, length;
