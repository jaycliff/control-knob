...
(function createControlKnobSetup(window, $, undef) {
    "use strict";
    ...
    $.createControlKnob = function createControlKnob(options) {
        ...
        function initializeParts() {
            ...
            $ck_bands_wrap.addClass('ck-bands-wrap').css('display: block;');
        ...
        function removeTransitionClass() {
            ...
            $ck_subwrap.removeClass('ck-transition');
            $ck_handle.off('transitionend', removeTransitionClass);
            transition_class_added = false;
        }
        function addTransitionClass() {
            //console.log('addTransitionClass');
            $ck_subwrap.addClass('ck-transition');
            $ck_handle.on('transitionend', removeTransitionClass);
        ...
        (function () {
            ...
            // Updates the knob UI
            updateControls = function updateControls(animate) {
                ...
                $ck_value.text(properties.value.toFixed(decimalDigitsLength(properties.step)));
                // highlight bands
            ...
            */
            ...
            ckWrapMetaControlHandler = (function () {
                var is_default_prevented = false, is_propagation_stopped = false, is_immediate_propagation_stopped = false, ck_anchor = $ck_anchor[0];
                ...
                return function ckWrapMetaControlHandler(event) {
                    ...
                    if (!ck_do_not_trigger_map[event_type]) {
                        ...
                        $control_knob_object.one(event_type, helper).triggerHandler(event_type);
            ...
            control_knob_object.enable = function enable() {
                if (disabled === true) {
                    disabled = false;
                    $control_knob_object.on('focus blur touchstart touchmove touchend touchcancel mousewheel DOMMouseScroll mousedown mousemove mouseup click keydown keyup keypress', ckEventHandler);
                    $ck_wrap
                        .removeClass('disabled')
                        .attr('tabindex', tab_index)
                        .on('focus blur touchstart touchmove touchend touchcancel mousewheel DOMMouseScroll mousedown mousemove mouseup click keydown keyup keypress', ckWrapMetaControlHandler)
            ...
            control_knob_object.disable = function disable() {
                if (disabled === false) {
                    disabled = true;
                    $control_knob_object.off('focus blur touchstart touchmove touchend touchcancel mousewheel DOMMouseScroll mousedown mousemove mouseup click keydown keyup keypress', ckEventHandler);
                    if (active) {
                        docWinEventHandler();
                    }
                    $ck_wrap
                        .addClass('disabled')
                        .removeAttr('tabindex')
                        .off('focus blur touchstart touchmove touchend touchcancel mousewheel DOMMouseScroll mousedown mousemove mouseup click keydown keyup keypress', ckWrapMetaControlHandler)
                        .on('mousedown touchstart', enableDisableAid);
                    removeTransitionClass();
                }
                return control_knob_object;
            };
            control_knob_object.refresh = updateControls;
